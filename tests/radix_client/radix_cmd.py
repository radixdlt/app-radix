...
from radix_client.radix_cmd_builder import RadixCommandBuilder, InsType
from radix_client.button import Button
from radix_client.exception import DeviceException
...
class RadixCommand:
    def __init__(self, transport: Transport, debug: bool = False) -> None:
        ...
        self.builder = RadixCommandBuilder(debug=debug)
        self.debug = debug
    ...
    def sign_tx(
        self, bip32_path: str, transaction: Transaction, button: Button
    ) -> Tuple[int, bytes]:
        sw: int
        response: bytes = b""
